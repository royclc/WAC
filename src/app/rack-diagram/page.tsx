...
export default function RackDiagramPage() {
  ...
  const ROW_MAX = 10 // max racks per visual line
  const THUMB_W = 185
  const THUMB_GAP = 12

  // Group racks by row_name, then chunk into sub-rows of ROW_MAX
  const rackRows = useMemo(() => {
    ...
    const result: { rowName: string; totalInRow: number; racks: Rack[]; subIndex: number }[] = []
    map.forEach((rowRacks, rowName) => {
      const total = rowRacks.length
      for (let i = 0; i < rowRacks.length; i += ROW_MAX) {
        result.push({ rowName, totalInRow: total, racks: rowRacks.slice(i, i + ROW_MAX), subIndex: Math.floor(i / ROW_MAX) })
      }
    })
    return result
  }, [racks])

  // Max racks in any single visual line (capped at ROW_MAX)
  const maxPerLine = useMemo(() => {
