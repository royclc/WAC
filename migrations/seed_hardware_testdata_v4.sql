...
-- ── 類別（含 icon + color）──
INSERT INTO hardware_categories (key, label, icon, color, sort_order) VALUES
...
INSERT INTO hardware_models (category_id, name) VALUES
  ...
  -- security
  ((SELECT id FROM hardware_categories WHERE key='security'), 'Fortinet FG-200F'),
  ((SELECT id FROM hardware_categories WHERE key='security'), 'Fortinet FG-600E'),
  ((SELECT id FROM hardware_categories WHERE key='security'), 'Palo Alto PA-850');
...
-- ===== A-01 Web LB =====
INSERT INTO hardware_assets (name,category_key,model,vendor,ip_address,remote_ip,location,description,is_active,rack_u_start,rack_u_size) VALUES
